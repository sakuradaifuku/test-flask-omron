...
class BasicProcess():
    ...
    def get15minConsumedCaloire(self, DB_data): # ここでちょっと挙動不審あり．動かなくなったら再デプロイして．
        ...
        for record in DB_data:
            dt = record["datetime"]
            time = "{0}/{1}/{2}-{3}:{4}".format(str(dt.year).zfill(2), str(dt.month).zfill(2), str(dt.date).zfill(2), str(dt.hour).zfill(2), str(dt.minute).zfill(2)) # 0で2桁合わせし，JS側で日付・時刻を分割しやすいように「-」を区切り文字にする
